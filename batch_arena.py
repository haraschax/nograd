...
class Players():
  ...
  def play(self, boards, test=False):
    ...
    if test:
      ...
      noise = torch.randn_like(boards.half())
  ...
  def mate(self, init_credits=INIT_CREDS):
    ...
    log_mutation_rates = torch.clamp(self.params['mutuation'].sum(dim=2), -15, 0)
    ...
    for key in self.params:
      ...
      mutation_rate_full = mutation_rates[:,cnt].reshape(unsqueezed_shape).expand(shape)
  ...
  def avg_log_mutuation(self):
    return self.params['mutuation'].sum(dim=2).mean().half().item()
...
def train_run(name='', init_credits=INIT_CREDS, embed_n=EMBED_N, bs=BATCH_SIZE):
  ...
  params = {'input': torch.randn((BATCH_SIZE*2, BOARD_SIZE*4, EMBED_N), dtype=torch.float16, device=DEVICE),
            'bias': torch.randn((BATCH_SIZE*2, EMBED_N), dtype=torch.float16, device=DEVICE),
            ...
            'output': torch.randn((BATCH_SIZE*2, EMBED_N, BOARD_SIZE), dtype=torch.float16, device=DEVICE),
            'mutuation': torch.randn((BATCH_SIZE*2, 4, MUTATION_PARAMS_SIZE), dtype=torch.float16, device=DEVICE)}
...
if __name__ == '__main__':
  for i in [1,2,4,8,16]:
    ...
    size_factor = 8
    name = f'run2_{i}'
    train_run(name=name, init_credits=init_credits, embed_n=size_factor*16, bs=10000*8//size_factor)
