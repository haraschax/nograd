...
class Players():
  ...
  def run_dna(self, dna_by_gene, input_vector):
    ...
    dna = rearrange(dna_by_gene, 'b (k i j)-> b k i j', k=1, i=LAYERS)
...
def play_games(games, x_players, o_players, test=False):
  ...
  if not test:
    for player in player_dict:
      #player_dict[player].credits[(games.winners == player) & (games.illegal_movers == PLAYERS.NONE)] += 2
...
def train_run(name='', embed_n=EMBED_N, bs=BATCH_SIZE):
  ...
  BATCH_SIZE = bs
  params = {}
  params['dna'] = torch.zeros((BATCH_SIZE*2, 2*(STATE_SIZE*(STATE_SIZE+1))), dtype=torch.float, device=DEVICE).uniform_(-1, 1)
